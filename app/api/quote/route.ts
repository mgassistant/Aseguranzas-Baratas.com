...
// aseguranzas-baratas tenant is created by the dev team.
const TENANT_ID =
  process.env.BROKERIQ_TENANT_ID || "a48b4bbb-0a1a-4cef-bb21-56c7bf94f64e";
const SOURCE = "aseguranzas-baratas.com";
const LEAD_TYPE = "auto_insurance_es";
...
const NOTIFY_FROM =
  ...
  "Aseguranzas Baratas <support@aseguranzas-baratas.com>";
...
export async function POST(req: NextRequest) {
  ...
  if (!isPartial) {
    ...
    if (!name || name.length < 2) missing.push("nombre");
    if (phone.length < 10) missing.push("teléfono");
    if (!email.includes("@") || !email.includes(".")) missing.push("correo");
    if (missing.length > 0) {
      return NextResponse.json(
        { error: `Faltan datos: ${missing.join(", ")}`, missing },
        { status: 400 }
      );
    }
  }
  ...
  // 2) Agency email via Resend (fail-open).
  const subjectPrefix = isPartial ? "[PARTIAL LEAD] " : "";
  const html = `
    <h2>${subjectPrefix}Nueva solicitud — Aseguranzas-Baratas.com</h2>
    <table cellpadding="6" style="border-collapse:collapse;font-family:sans-serif">
      ...
      <tr><td><b>Vehículo</b></td><td>${esc(vehicle) || "—"}</td></tr>
      <tr><td><b>SR-22</b></td><td>${esc(sr22) || "—"}</td></tr>
      <tr><td><b>Estado</b></td><td>${esc(state) || "—"}</td></tr>
      <tr><td><b>Tipo</b></td><td>${isPartial ? "Parcial / abandonado" : "Completo"}</td></tr>
    </table>
    <p style="color:#888;font-size:12px">Fuente: ${SOURCE}</p>`;
  const emailPromise = sendEmail(
    `${subjectPrefix}Nueva cotización — ${name || email || phone || "sin datos"}`,
    html
  );
